...
export default function EmotionalMap() {
  ...
  const getInsight = () => {
    if (MOCK_ENTRIES.length > 20) return "Has estado observando tus emociones de cerca últimamente.";
    if (MOCK_ENTRIES.some(e => e.emotion === 'sadness')) return "Está bien tener días nublados. Forman parte del paisaje.";
      ...
      <div className="max-w-4xl w-full z-10 flex flex-col items-center">
        ...
        <div ref={constellationRef} className="flex flex-wrap justify-center gap-4 md:gap-6 lg:gap-8 max-w-2xl mx-auto mb-20">
          {MOCK_ENTRIES.map((entry, index) => {
            const styleInfo = EMOTION_PALETTE[entry.emotion];
            const baseSize = 40;
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4 sm:p-8">
          ...
          >
            ...
            <div className="flex items-center gap-4 mb-8">
               <div className={`w-12 h-12 rounded-full ${EMOTION_PALETTE[selectedEntry.emotion].bg} opacity-80 shadow-sm`} />
               <div className="text-sm font-medium opacity-60">
                 Resonancia: {EMOTION_PALETTE[selectedEntry.emotion].label.split(' (')[0]}
               </div>
            </div>
